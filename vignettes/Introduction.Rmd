...
rec <- recipe(time + status ~ ., data = surv_train) %>%
  add_role(time, new_role = "surv_time") %>%
  add_role(status, new_role = "surv_event") %>%
  add_role(status, new_role = "case_stratum")
...
`require`
  : names of installed packages to load during parallel execution of resampling algorithms [default: ``c("MachineShop", "recipes", "survival")``].
