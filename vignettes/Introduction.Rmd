...
`packages`
  : Character vector of source packages required to use the model.  These need only be installed with the `install.packages` function or by equivalent means; but need not be loaded, for example, with the `library` function.
...
`maximize`
  : Logical indicating whether larger values of the metric correspond to better predictive performance.
