...
```{r}
## Resample estimation
(perf <- resample(fo, lung, GBMModel, control))
...
perf <- resample(type ~ ., df, GBMModel)
summary(perf)
```
...
perf <- resample(Species ~ ., df, GBMModel)
summary(perf)
```
...
library(MASS)
perf <- resample(medv ~ ., Boston, GBMModel)
summary(perf)
```
...
library(survival)
perf <- resample(Surv(time, status) ~ ., lung, GBMModel)
summary(perf)
```
...
gbmfit <- fit(medv ~ ., data = Boston, GBMModel)
varimp(gbmfit)
```
