...
A named list of supplied metrics can be obtained with the `metricinfo` function, and includes a descriptive `"label"` for each, whether to `"maximize"` the metrics for better performance, their function `"arguments"`, and supported observed and predicted response variable `"types"`.  Function `metricinfo` may be called with one or more metric functions, an observed response variable, an observed and predicted response variable pair, response variable types, or resampled output; and will return information on all matching metrics.
