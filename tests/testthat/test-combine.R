...
test_that("object combinations", {
  ...
  with_parallel({
    ...
    x <- runif(10, 0, 100)
    binom <- BinomialVector(x, size = 100)
    disc <- DiscreteVector(x)
    negbinom <- NegBinomialVector(x)
    pois <- PoissonVector(x)
    
    expect_s4_class(c(binom), "BinomialVector")
    expect_s4_class(c(binom, binom), "BinomialVector")
    ...
    expect_s4_class(c(disc, binom), "DiscreteVector")
